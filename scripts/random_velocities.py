...
class RndVelocityGen():
    def __init__(self):
        ...
        self.vel.linear.x = rospy.get_param('/init_speed',0.1) # m/s
        self.vel.angular.z = rospy.get_param('/init_omega',0.05) # rad/s
        ...
        self.linear_vel_x_max = rospy.get_param('~max_speed',1.2)
        self.angular_vel_z_max = rospy.get_param('~max_omega',0.5)
        # Set max value for max time interval between velocity changes,
        # min is set to 1
        self.max_interval = rospy.get_param('~max_interval',15)
        print ("Max velocities: {}, {} - Max interval {}".format(self.linear_vel_x_max,
                                                                self.angular_vel_z_max,
                                                                self.max_interval))
        print ("Name resolve: init_speed: {} - max_speed: {}".format(rospy.resolve_name('/init_speed'),
        rospy.resolve_name('~max_speed')))
        print ("Node's namespace: ", rospy.get_namespace())
